...
struct keybind_state
{
    ...
    ImVec4 text_color = ImVec4(1.f, 1.f, 1.f, 1.f);
    float alpha = 0.f;
};

bool ImGui::Keybind(const char* label, int* key, bool widget_label) {
    ...
    SetCursorPos(GetCursorPos() - ImVec2(0, 10));

    const ImGuiID id = window->GetID(label);
    const float size = GetWindowWidth() - 36;
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::tab_text_active : hovered_rect ? c::tab_text_hov : c::text, g.IO.DeltaTime * 6.f);
    ...
    if (widget_label) window->DrawList->AddText(rect.Min + ImVec2(1, 4), GetColorU32(it_anim->second.text_color), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));

    window->DrawList->AddRectFilled(key_rect.Min + ImVec2(CalcItemWidth() - 21, -1), key_rect.Max - ImVec2(2, 12), GetColorU32(c::keybind_background), style.FrameRounding);
    window->DrawList->AddRect(key_rect.Min + ImVec2(CalcItemWidth() - 21, -1), key_rect.Max - ImVec2(2, 12), GetColorU32(c::keybind_border), style.FrameRounding);
...
void ImGui::TextColoredTabs(const ImVec4& col, const char* ico, const char* fmt...)
{
    va_list args;
    va_start(args, fmt);

    PushFont(ico_1);
    TextColoredV(col, ico, args);
    PopFont();

    ImGui::SameLine(0, 10); SetCursorPos(GetCursorPos() - ImVec2(0, 1));
...
void ImGui::TextColoredSubTabs(const ImVec4& col, const char* fmt, ...)
{
    ImGuiWindow* window = GetCurrentWindow();

    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(fmt);
    ...
    const ImRect bb(pos, pos);

    va_list args;
    va_start(args, fmt);
    SetCursorPos(GetCursorPos() - ImVec2(0, 1));

    const int vtx_idx_2 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    PushFont(ubu_1);
    TextColoredV(col, fmt, args);
    PopFont();
    ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_2, ImGui::GetWindowDrawList()->VtxBuffer.Size, bb.Min, bb.Max + ImVec2(50, 20), GetColorU32(c::tab_push_gradient), GetColorU32(c::tab_pop_gradient));
...
struct button_state
{
    float text_alpha = 0.3f;
    float rect_alpha = 0.f;
    bool active = false;
    ImVec4 text, bg;
};
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (size.x <= 0.0f)
        size.x = GetWindowWidth() - (g.Style.WindowPadding.x * 2);
    if (size.y <= 0.0f)
        size.y = 27;
    ...
    it_anim->second.text_alpha = ImClamp(it_anim->second.text_alpha + (4.f * g.IO.DeltaTime * (it_anim->second.active ? 1.f : -1.f)), 0.3f, 1.f);
    it_anim->second.rect_alpha = ImClamp(it_anim->second.rect_alpha + (5.f * g.IO.DeltaTime * (it_anim->second.active ? 1.f : -1.f)), 0.0f, 1.f);
    it_anim->second.bg = ImLerp(it_anim->second.bg, it_anim->second.active ? ImColor(c::accent_color) : hovered ? ImColor(24,24,26) : ImColor(24, 24, 26), g.IO.DeltaTime * 5.f);
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.active ? ImColor(255, 255, 255) : hovered ? ImColor(150, 150, 150) : ImColor(80, 80, 80), g.IO.DeltaTime * 4.f);

    if (it_anim->second.rect_alpha >= 0.9f)
        it_anim->second.active = false;

    window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(it_anim->second.bg), 3);
...
struct tab_state {
    ...
    ImVec4 background_color, background_fill;
    ImVec4 glow_color;
    ImVec4 text_color;

};

bool ImGui::Tab(const char* ico, const char* label, bool selected, const ImVec2& size_arg, bool tabs, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? c::tab_text_active : hovered ? c::tab_text_hov : c::tab_text, g.IO.DeltaTime * 6.f);
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, selected ? c::tab_active : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.background_fill = ImLerp(it_anim->second.background_fill, selected ? c::accent_color : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.glow_color = ImLerp(it_anim->second.glow_color, selected ? c::accent_color : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);

    if (tabs) {

        ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(10,4), bb.Max - ImVec2(17, -4), GetColorU32(it_anim->second.background_color), 3);
        ImGui::GetWindowDrawList()->AddRectFilled(bb.Max - ImVec2(20, 22), bb.Max + ImVec2(-18, -6), GetColorU32(it_anim->second.background_fill), 10, ImDrawCornerFlags_Left);

        ImGui::GetWindowDrawList()->AddText(ico_0, 23, ImVec2(bb.Min.x + 20, bb.Max.y + style.FramePadding.y - 28), GetColorU32(it_anim->second.text_color), ico);
        ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 51, bb.Max.y + style.FramePadding.y - 26), GetColorU32(it_anim->second.text_color), label);
    }
    else {

        ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(size_arg.x / 3, 38), bb.Max - ImVec2(size_arg.x / 3, -15), GetColorU32(it_anim->second.background_color), 0);
        ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(size_arg.x / 3, 28), bb.Max - ImVec2(size_arg.x / 3, -15), ImColor(32, 31, 37, 0), ImColor(32, 31, 37, 0), GetColorU32(it_anim->second.glow_color), GetColorU32(it_anim->second.glow_color));

        PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
        PushFont(ubu_child);
        RenderTextClipped(bb.Min + style.FramePadding + ImVec2(3, 1), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopFont();
        PopStyleColor();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    float alpha = 1.0f;
    if ((axis == ImGuiAxis_Y) && bb_frame_height < g.FontSize + g.Style.FramePadding.y * 0.0f)
        alpha = ImSaturate((bb_frame_height - g.FontSize) / (g.Style.FramePadding.y * 0.0f));
    if (alpha <= 0.0f)
        return false;
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 1.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(IM_FLOOR((bb_frame_height - 1.0f) * 0.5f), 0.0f, 3.0f)));
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, GetColorU32(c::scroll_background), window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(1, 3), grab_rect.Max + ImVec2(1, -3), GetColorU32(c::scroll_keep), style.ScrollbarRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::accent_color : c::circle_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::checkbox_active : c::checkbox_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? c::text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);
    it_anim->second.slow_circle = ImLerp(it_anim->second.slow_circle, *v ? -13.f : 0.f, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2((w - 9), 0), check_bb.Max + ImVec2(w + 11, 7.5f), GetColorU32(it_anim->second.background), 100);
    GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2((w + 1 - it_anim->second.slow_circle), 9.5f), 5.5f, GetColorU32(it_anim->second.circle));
    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y + 3.5f), GetColorU32(it_anim->second.text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags, bool multi)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 13));
    ...
    // переписываем опен комбо систему
    ...
    // размер комбо == кол-во элементов * на размер одного элемента (val * 17), также надо учитывать паддинги, поэтому прибавляем ещё 13
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? 17 * val * 2 + 10 : 0.f, g.IO.DeltaTime * 8.f);

    it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.opened_combo ? c::text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 21, bb.Max.y), GetColorU32(c::combo_background), style.FrameRounding);
    ...
    PushFont(ico_0);
    RenderTextClipped(bb.Min + ImVec2(w + 25, 8), ImVec2(value_x2 + 20, bb.Max.y + 5), "J", NULL, NULL);
    PopFont();

    PopStyleColor();

    window->DrawList->AddText(ImVec2(bb.Max.x - w - 54, bb.Min.y - 25), GetColorU32(it_anim->second.text_color), label);

    // проверка на открытое комбо
    ...
    // необходимые флаги, теперь мы создаем не popup, а обычное окно.
    ...
    PushStyleColor(ImGuiCol_WindowBg, GetColorU32(c::combo_background));
    ...
    // если у нас мультикомбо, то нам не надо закрывать его при нажатии на элемент, поэтому делаем проверку
    if (multi)
    {
...
void ImGui::EndCombo()
{
    // т.к мы создаём не popup, а обычное окно, то и закрывать его должны не с помощью EndPopup, а с помощью End
    End();
}
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_usable_pos_max = bb.Max[axis] - grab_padding - grab_sz * 0.3f;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(110, - 9), window->DC.CursorPos + ImVec2(w + 65, -6));
    const ImRect frame_bb2(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 12.f);
    it_anim->second.hint_slider = ImLerp(it_anim->second.hint_slider, IsItemHovered() ? 1.f : 0.f, g.IO.DeltaTime * 35.f);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(10, 0), GetColorU32(c::slider_background), 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        it_anim->second.slow = ImLerp(it_anim->second.slow, grab_bb.Min.x + 8 - frame_bb.Min.x, g.IO.DeltaTime * 20.f);

        const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(0, -2), frame_bb.Min + ImVec2(it_anim->second.slow - 7, 1), GetColorU32(c::accent_color), 100, ImDrawFlags_RoundCornersLeft);
        ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, frame_bb.Min, frame_bb.Max, GetColorU32(c::accent_color, 1), GetColorU32(c::accent_color, 1));

        const int vtx_idx_2 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        window->DrawList->AddCircleFilled(frame_bb.Min + ImVec2(it_anim->second.slow - 2, 0.5f), 5, GetColorU32(c::accent_color), 100);
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    RenderTextClipped(frame_bb2.Min + ImVec2(20, -20), frame_bb2.Min + ImVec2(100, 0), value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));
    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w - 65, frame_bb.Min.y - 8), label);
    PopStyleColor();
...
namespace ImGui
{
    ...
    bool drawGrabber(const char* id, ImVec2& pos, t& input, const t& min, const t& max, float width, float height, int idnum)
    {
        ...
        bool changed = false;
        bool hovered = false;
        float current_draw_bar = input / scale + 4;
        static float previous_delta;
        hovered = false;
        SetCursorScreenPos({ pos.x + current_draw_bar - 55, pos.y });
        std::string nid = id + std::to_string(idnum);
        PushID(nid.c_str());
        ImGui::InvisibleButton(nid.c_str(), { bar_width + 89, height + 10 });
        ...
        static std::map<ImGuiID, rangeslider_state> anim;
        auto it_anim = anim.find(ImGui::GetID(id));

        if (it_anim == anim.end())
        {
            anim.insert({ ImGui::GetID(id), rangeslider_state() });
            it_anim = anim.find(ImGui::GetID(id));
        }

        it_anim->second.slow = ImLerp(it_anim->second.slow, current_draw_bar, g.IO.DeltaTime * 15.f);
        it_anim->second.circle_color = ImLerp(it_anim->second.circle_color, IsItemActive() ? c::circle_push_gradient : hovered ? c::circle_push_gradient : c::circle_push_gradient, g.IO.DeltaTime * 19.f);
        it_anim->second.circle_radius = ImLerp(it_anim->second.circle_radius, IsItemActive() ? 8.0f : 8.0f, g.IO.DeltaTime * 8.f);
        it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::text_active : hovered ? c::text_hov : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);

        win->DrawList->AddCircleFilled({ pos.x + it_anim->second.slow, pos.y + 38 }, it_anim->second.circle_radius, GetColorU32(it_anim->second.circle_color), 100);

        PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
        RenderTextClipped(pos - ImVec2(0, 20), ImVec2(pos.x + 228, pos.y + 50), std::to_string(input).c_str(), NULL, NULL, ImVec2(1.0f, 0.5f));
        PopStyleColor();
    ...
    bool RangeSlider(const char* id, const t& min, const t& max, t& low, t& high)
    {
        ...
        SetCursorPos(GetCursorPos() + ImVec2(0, 0));

        ImGuiWindow* win = GetCurrentWindow();
        ImGuiContext& g = *GImGui;
        ImGuiStyle& style = g.Style;

        float min_scalar = low;
        float max_scalar = high;

        const float button_size = GetFrameHeight();
        float width =  CalcItemWidth() + 111 - (button_size + style.ItemInnerSpacing.x) * 2;
        float height = ImGui::GetTextLineHeightWithSpacing();
        bool changed = false;
        SetNextItemWidth(ImMax(0.0f, width));
        ImVec2 pos = win->DC.CursorPos;
        ImVec2 end = { pos.x + width + 7, pos.y + height };
        float scale = (max / width);
        ...
        win->DrawList->AddRectFilled(pos + ImVec2(0, 34), end - ImVec2(0, 0), GetColorU32(c::slider_background), 100);

        it_anim->second.min_slow = ImLerp(it_anim->second.min_slow, min_scalar, g.IO.DeltaTime * 20.f);
        it_anim->second.max_slow = ImLerp(it_anim->second.max_slow, max_scalar, g.IO.DeltaTime * 20.f);

        const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        win->DrawList->AddRectFilled({ pos.x + it_anim->second.min_slow / scale, pos.y + 34 }, { pos.x + it_anim->second.max_slow / scale, pos.y + height - 0 }, GetColorU32(c::accent_color), 100); //bar between the 2 grabbers
        ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, { pos.x + it_anim->second.min_slow / scale, pos.y + 35 }, { pos.x + it_anim->second.max_slow / scale, pos.y + height - 0 }, GetColorU32(c::accent_color), GetColorU32(c::accent_color));

        PushStyleColor(ImGuiCol_Text, GetColorU32(c::text));
        RenderText(ImVec2(pos.x + 0, pos.y + 5), id);
        PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGui::SetCursorPos(GetCursorPos() + ImVec2(0, 20));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 54, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 7.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        ...
        const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(c::input_push_gradient), true, style.FrameRounding);
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(4, 7), rect.Max + ImVec2(4, 7), bg_color);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(4, 7), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 2, 10), frame_bb.Min + ImVec2(it_anim->second.value + 3, 26), GetColorU32(ImGuiCol_Text));
            }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(4, 7), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);

    window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() - 54, frame_bb.Min.y - 26), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        SetNextWindowSize(ImVec2(300, 230));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        SetCursorPos(GetCursorPos() - ImVec2(31, 5));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size + ImVec2(CalcItemWidth() + 35, 0));
    ...
    float grid_step = ImMin(size.x, size.y) / 1.99f;
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min + ImVec2(CalcItemWidth() + 35, -5), bb_inner.Max - ImVec2(0, 5), GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min + ImVec2(CalcItemWidth() + 35, -5), bb_inner.Max - ImVec2(0, 5), GetColorU32(col_source), rounding);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_anim = ImClamp(it_anim->second.text_anim + (50.f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.f)), 0.f, 5.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? c::accent_color : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);
    it_anim->second.fill_color = ImLerp(it_anim->second.fill_color, selected ? c::accent_color : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    RenderTextClipped(text_min + ImVec2(10 + it_anim->second.text_anim, -4.f), text_max + ImVec2(30 + it_anim->second.text_anim, -4.2f), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();

    window->DrawList->AddRectFilled(text_min + ImVec2(-4, -4), text_min + ImVec2(0, 12), GetColorU32(it_anim->second.fill_color), 10, ImDrawCornerFlags_Right);
