...
struct IMGUI_API ImRect
{
    ...
    void        Floor() { Min.x = ImTrunc(Min.x); Min.y = ImTrunc(Min.y); Max.x = ImTrunc(Max.x); Max.y = ImTrunc(Max.y); }
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MIN                     50
...
//#endif
    ...
    IMGUI_API void          TextColoredTabs(const ImVec4& col, const char* ico, const char* fmt...);
    IMGUI_API void          TextColoredSubTabs(const ImVec4& col, const char* fmt, ...);


        template <class t>
        IMGUI_API bool RangeSlider(const char* id, const t& min, const t& max, t& low, t& high);
        IMGUI_API bool RangeSliderInt(const char* id, const int& min, const int& max, int& low, int& high);
        IMGUI_API bool RangeSliderFloat(const char* id, const float& min, const float& max, float& low, float& high);
    ...
    IMGUI_API bool          Keybind(const char* label, int* key, bool widget_label);
